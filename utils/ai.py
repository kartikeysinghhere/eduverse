...
def get_ai_response(prompt):
    ...
    # Check for specific student searches
        ...
        if matched_student is not None:
            status = "At Academic Risk ⚠️" if matched_student["risk"] == 1 else "Good Standing ✅"
            fallback_response = (
    ...
    if not fallback_response:
        if "what is eduverse" in lower_prompt or "eduverse kya hai" in lower_prompt:
            ...
            fallback_response = (
                ...
                f"- **Students At-Risk (risk=1):** {at_risk_count} students ⚠️\n"
                f"- **Top Performers:** {top_5_str} 🏆"
            )

    # 3. Call Groq API if Key is present
