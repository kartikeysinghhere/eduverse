...
def generate_score(study_hours, max_score):
    """
    Base performance driven by study hours (scale 0.5 to 0.95)
...
def pct_to_gpa(pct):
    """
    Converts percentage score to GPA using standard conversion scale.
    No student should have GPA below 1.5.
    """
    if pct >= 93:
        gpa = 4.0
    elif pct >= 90:
...
def generate_dataset(n=500, target_min=3.20, target_max=3.40):
    ...
    # Feedback loop to automatically adjust mean study hours to hit the target GPA
    mean_sh = 24.0
    attempts = 0
    
    while attempts < 100:
        np.random.seed(42 + attempts)  # Shifting seed slightly to keep natural variation
